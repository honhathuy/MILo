...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y) renderCUDA_apply_weights(
    ...
    float *__restrict__ weights, // added
...
)
{
  ...
  if (inside)
  {
    ...
    {
      // printf("pixid %d\n", pix_id);
      // printf("image_weights %d\n", image_weights.size(0));
      // printf("image_height %d\n", image_weights.size(1));
      // printf("channel colors %f\n", image_weights[ch * H * W + pix_id]);
      clr[ch] = image_weights[ch * H * W + pix_id];
      // if (clr[ch] > 0.0)
      // printf("Csdf = %f %d %d %d %d %d\n", image_weights[ch * H * W + pix_id], ch * H * W + pix_id, ch, H, W, pix_id);
    }
  }
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
  {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
    {
      ...
      for (int ch = 0; ch < CHANNELS; ch++)
      {
        ...
        // {
        //   printf("image_weights %f\n", C[ch]);
        //   atomicAdd(weights + (collected_id[j] * CHANNELS + ch), image_weights[ch * H * W + pix_id]);
        //   atomicAdd(cnt + collected_id[j], 1);
        // }
      }
      T = test_T;
...
void APPLY_WEIGHTS::preprocess(
    ...
    const dim3 grid, uint32_t *tiles_touched, bool prefiltered)
{
  preprocessCUDA_apply_weights<NUM_CHAFFELS><<<(P + 255) / 256, 256>>>(
