...
def load_gc_args(parser):
    ...
    parser.add_argument('--sig_col_neigh', type=float, default=5)
    ...
    parser.add_argument('--user_weight_term', type=float, default=20)
...
def graphcut_segmentation(args, root_path, graphcutparams, weights_source, weights_sink, gaussians, gaussians_source, 
    ...
    keep_mask = torch.from_numpy(remove == 0).bool().cuda()
    gaussians_copy.remove_low_score_gaussians(keep_mask)

    # gaussians_copy.save_ply(root_path + '/graphcut/gaussians_source.ply')
    # torch.save(torch.from_numpy(remove), root_path + '/graphcut/remove_source.pt')
