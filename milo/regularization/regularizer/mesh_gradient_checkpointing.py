...
def compute_mesh_regularization(
    ...
    if iteration >= config["start_iter"]:
        ...
        def _differentiable_mesh_pass(
            voronoi_points, delaunay_tets, current_voronoi_sdf, voronoi_scale,
            viewpoint_cam, viewpoint_idx, config, mesh_renderer
        ):
            # --- Marching Tetrahedra ---
            ...
            if config["collapse_large_edges"]:
                min_end_points = end_points[
                    torch.arange(end_points.shape[0], device=end_points.device), 
                    end_sdf.argmin(dim=1).flatten()
                ]
            ...
            mesh_render_pkg = mesh_renderer(
                ...
                cameras=[viewpoint_cam], # Use the current cam directly
            )
            mesh_depth = (
                mesh_render_pkg["depth"].squeeze() 
                if config["use_depth_loss"] 
                else torch.zeros(viewpoint_cam.image_height, viewpoint_cam.image_width, device=verts.device)
            )  # (H, W)
            mesh_normal_view = (
                mesh_render_pkg["normals"].squeeze() @ viewpoint_cam.world_view_transform[:3,:3] 
                if config["use_normal_loss"] 
                else torch.zeros(viewpoint_cam.image_height, viewpoint_cam.image_width, 3, device=verts.device)
            )  # (H, W, 3)
            rasterization_mask = mesh_depth > 0.  # (H, W)
            return mesh_depth, mesh_normal_view, rasterization_mask, verts, faces

        # Use checkpointing to save memory
        mesh_depth, mesh_normal_view, rasterization_mask, verts, faces = checkpoint.checkpoint(
            _differentiable_mesh_pass,
            voronoi_points, delaunay_tets, current_voronoi_sdf, voronoi_scale,
            viewpoint_cam, viewpoint_idx, config, mesh_renderer,
            use_reentrant=False # Safer for modern PyTorch
        )
